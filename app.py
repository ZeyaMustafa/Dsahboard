...
app.config['LIVE_SQL_CACHE_TTL_SECONDS'] = int(os.environ.get('LIVE_SQL_CACHE_TTL_SECONDS', 120))
app.config['MAX_CONTENT_LENGTH'] = 64 * 1024 * 1024  # 64 MB upload limit
_is_https = os.environ.get('HTTPS', '').lower() in ('1', 'true', 'on')
...
AI_DECISION_CACHE = {}
SALES_DATA_CACHE = {}
ANALYTICS_RUNTIME_CACHE = {}
LIVE_SQL_DATA_CACHE = {}
LIVE_SQL_METADATA_CACHE = {}
LIVE_SQL_DATA_CACHE_LOCK = threading.RLock()
...
SQL_SERVER_DEFAULT_LOOKBACK_DAYS = int(os.environ.get('SQL_SERVER_DEFAULT_LOOKBACK_DAYS', 90))
SQL_SERVER_FAILURE_COOLDOWN_SECONDS = int(os.environ.get('SQL_SERVER_FAILURE_COOLDOWN_SECONDS', 60))
SQL_SERVER_LIVE_FAILURE = {'last_failed_at': 0.0, 'message': ''}
BUNDLE_SIGNAL_LOOKBACK_DAYS = int(os.environ.get('BUNDLE_SIGNAL_LOOKBACK_DAYS', 15))
...
# Configure OAuth — allow HTTP only in local/dev environments
if not _is_https:
...
# ----------- Simple in-process rate limiter for auth routes -----------
_AUTH_ATTEMPTS: dict = {}
_AUTH_ATTEMPTS_LOCK = threading.Lock()
_AUTH_RATE_WINDOW = 300  # 5 minutes
_AUTH_RATE_LIMIT = 20    # max attempts per window per IP
...
def inject_template_globals():
    ...
    if current_user.is_authenticated:
        try:
            is_live_sql = get_integration_setting('integration_mode', '') == 'sql_server_live'
        except Exception:
            pass
    return {
        'current_year': datetime.now().year,
        'data_source_live_sql': is_live_sql,
    }
...
def clear_runtime_caches():
    """Clear all in-process caches after source data or business settings change."""
    AI_DECISION_CACHE.clear()
    SALES_DATA_CACHE.clear()
    ANALYTICS_RUNTIME_CACHE.clear()
    with LIVE_SQL_DATA_CACHE_LOCK:
        LIVE_SQL_DATA_CACHE.clear()
        LIVE_SQL_METADATA_CACHE.clear()

def source_cache_signature():
    mode = get_integration_setting('integration_mode', 'sample_csv')
    active_csv = configured_sales_csv_path()
    csv_signature = None
    if active_csv and os.path.exists(active_csv):
        stat = os.stat(active_csv)
        csv_signature = (os.path.abspath(active_csv), stat.st_mtime_ns, stat.st_size)
    sql_signature = (
        get_integration_setting('sql_server_host', ''),
        get_integration_setting('sql_server_port', ''),
        get_integration_setting('sql_server_database', ''),
        get_integration_setting('sql_server_auth_mode', ''),
        get_integration_setting('sql_server_username', ''),
        get_integration_setting('sql_server_driver', ''),
        get_integration_setting('sql_server_encrypt', ''),
        get_integration_setting('sql_server_trust_certificate', ''),
        hashlib.sha1((get_integration_setting('sql_server_query', '') or '').encode('utf-8')).hexdigest(),
    )
    return mode, csv_signature, sql_signature

def dataframe_cache_signature(df):
    if df is None or df.empty:
        return ('empty',)
    dates = pd.to_datetime(df['Date'], errors='coerce')
    sales_total = round(float(pd.to_numeric(df.get('Total_Sale', 0), errors='coerce').fillna(0).sum()), 2)
    profit_total = round(float(pd.to_numeric(df.get('Total_Profit', 0), errors='coerce').fillna(0).sum()), 2)
    product_count = int(df['Product_ID'].nunique()) if 'Product_ID' in df.columns else 0
    invoice_count = int(df['Invoice_No'].nunique()) if 'Invoice_No' in df.columns else 0
    return (
        int(len(df)),
        str(dates.min()),
        str(dates.max()),
...
def cached_dataframe_compute(cache_name, df, extra, factory):
    key = (cache_name, source_cache_signature(), dataframe_cache_signature(df), tuple(extra or ()))
    if key in ANALYTICS_RUNTIME_CACHE:
        return ANALYTICS_RUNTIME_CACHE[key]
    result = factory()
    if len(ANALYTICS_RUNTIME_CACHE) > 128:
        ANALYTICS_RUNTIME_CACHE.clear()
    ANALYTICS_RUNTIME_CACHE[key] = result
    return result

def cached_metrics(df, period_start=None, period_end=None):
    return cached_dataframe_compute('metrics', df, (period_start, period_end), lambda: build_metrics(df, period_start, period_end))

def cached_charts(df):
    return cached_dataframe_compute('charts', df, (), lambda: build_charts(df))

def cached_product_performance(df):
    return cached_dataframe_compute('product_performance', df, (), lambda: build_product_performance(df))

def cached_waste_reduction(df):
    return cached_dataframe_compute('waste_reduction', df, (), lambda: calculate_waste_reduction(df))

def cached_pricing_opportunities(df):
    return cached_dataframe_compute('pricing_opportunities', df, (), lambda: calculate_dynamic_pricing_opportunities(df))

def cached_cross_selling(df):
    return cached_dataframe_compute('cross_selling', df, (), lambda: calculate_cross_selling_opportunities(df))
...
def prewarm_dashboard_cache():
    try:
        with app.test_request_context('/'):
            ...
            metrics = cached_metrics(filtered_df, range_context['start_date'], range_context['end_date'])
            charts = cached_charts(filtered_df)
            product_performance = cached_product_performance(filtered_df)
            cached_waste_reduction(filtered_df)
            cached_pricing_opportunities(filtered_df)
            cached_cross_selling(filtered_df)
            build_seller_decision_intelligence(filtered_df, {
                **range_context,
                **metrics,
                **charts,
                **product_performance,
        ...
        app.logger.warning("Dashboard cache prewarm failed: %s", exc)

def start_dashboard_cache_prewarm():
    threading.Thread(target=prewarm_dashboard_cache, name='dashboard-cache-prewarm', daemon=True).start()
...
def normalize_canonical_sales_dataframe(df):
    required_columns = [
        'Date', 'Product_ID', 'Product_Name', 'Category', 'Units_Sold',
        'Unit_Price', 'Profit_Per_Unit', 'Total_Sale', 'Total_Profit'
    ]
    ...
    for column in ['Units_Sold', 'Unit_Price', 'Profit_Per_Unit', 'Total_Sale', 'Total_Profit']:
    ...
    normalized['Category'] = normalized['Category'].replace('', 'Uncategorized')

    for column in OPTIONAL_SALES_COLUMNS:
        if column not in normalized.columns:
            normalized[column] = None

    if 'Invoice_No' in normalized.columns:
        normalized['Invoice_No'] = normalized['Invoice_No'].fillna('').astype(str).str.strip()

    return normalized.sort_values('Date')

def configured_sales_csv_path():
    """Return the active CSV path only if it still exists.

    The active source is persisted in SQLite, so a deleted import file can
    otherwise come back as the selected source after a server restart.
    """
    configured_csv = get_integration_setting('active_sales_csv_path')
    if configured_csv:
        if os.path.exists(configured_csv):
            return configured_csv
        app.logger.warning("Configured sales CSV no longer exists: %s", configured_csv)
        set_integration_setting('active_sales_csv_path', '')
        if get_integration_setting('integration_mode') in {'csv', 'sql_server', 'sql_server_snapshot', 'sqlite_snapshot'}:
            set_integration_setting('integration_mode', 'sample_csv')
        clear_runtime_caches()

    default_csv = app.config.get('SALES_CSV_PATH')
    return default_csv if default_csv and os.path.exists(default_csv) else None

def sales_source_display_name(csv_path=None):
    mode = get_integration_setting('integration_mode', 'sample_csv')
    if mode == 'sql_server_live':
        settings = sql_server_settings()
        return f"Live SQL Server: {settings.get('host')} / {settings.get('database')}"
    if mode == 'sample_csv':
        return 'Sample CSV' if csv_path else 'Not configured'
    source_name = get_integration_setting('last_source_name', '')
    if source_name:
        return source_name
    if csv_path:
...
def read_csv_sales_data():
    configured_csv = configured_sales_csv_path()
    ...
    for csv_file in csv_files:
        try:
            ...
            cache_key = (os.path.abspath(csv_file), stat.st_mtime_ns, stat.st_size)
            normalized = SALES_DATA_CACHE.get(cache_key)
            if normalized is None:
                raw_df = pd.read_csv(csv_file)
                normalized = normalize_canonical_sales_dataframe(raw_df)
                if normalized is None:
                    normalized = normalize_sales_dataframe(raw_df, csv_file)
                if normalized is not None and not normalized.empty:
                    SALES_DATA_CACHE.clear()
                    SALES_DATA_CACHE[cache_key] = normalized
            if normalized is not None and not normalized.empty:
                frames.append(normalized.copy(deep=False))
        except Exception as exc:
...
def using_live_sql_server_source():
    if get_integration_setting('integration_mode') != 'sql_server_live':
        return False
    last_status = get_integration_setting('sql_server_last_status', '')
    if last_status.startswith('Live SQL Server read failed'):
        active_csv = configured_sales_csv_path()
        if active_csv and os.path.exists(active_csv):
            set_integration_setting('integration_mode', 'sql_server_snapshot')
            return False
    return True

def live_sql_server_in_cooldown():
    return (
        SQL_SERVER_LIVE_FAILURE['last_failed_at'] and
        time.time() - SQL_SERVER_LIVE_FAILURE['last_failed_at'] < SQL_SERVER_FAILURE_COOLDOWN_SECONDS
    )

def remember_live_sql_failure(exc):
    SQL_SERVER_LIVE_FAILURE['last_failed_at'] = time.time()
    SQL_SERVER_LIVE_FAILURE['message'] = str(exc)
    set_integration_setting('sql_server_last_status', f'Live SQL Server read failed; using snapshot: {exc}')
    active_csv = configured_sales_csv_path()
    if active_csv and os.path.exists(active_csv):
        set_integration_setting('integration_mode', 'sql_server_snapshot')

def clear_live_sql_failure():
    SQL_SERVER_LIVE_FAILURE['last_failed_at'] = 0.0
    SQL_SERVER_LIVE_FAILURE['message'] = ''
...
def ensure_inventory_products_for_sales(df):
    ...
    if 'Current_Stock' in df.columns:
        product_rows = product_rows.merge(
            df.groupby(['Product_ID', 'Product_Name', 'Category'], as_index=False)['Current_Stock'].max(),
            on=['Product_ID', 'Product_Name', 'Category'],
            how='left',
        )
    if 'Reorder_Level' in df.columns:
        product_rows = product_rows.merge(
            df.groupby(['Product_ID', 'Product_Name', 'Category'], as_index=False)['Reorder_Level'].max(),
            on=['Product_ID', 'Product_Name', 'Category'],
            how='left',
        )
...
def load_data():
    """Load sales records from the active source."""
    if using_live_sql_server_source() and not live_sql_server_in_cooldown():
        settings = sql_server_settings()
        try:
            end_date = datetime.now()
            start_date = end_date - timedelta(days=SQL_SERVER_DEFAULT_LOOKBACK_DAYS - 1)
            normalized = read_normalized_live_sql_sales(settings, start_date, end_date)
            if normalized is not None and not normalized.empty:
                clear_live_sql_failure()
                return normalized
        except Exception as exc:
            app.logger.warning("Could not read live SQL Server source: %s", exc)
            remember_live_sql_failure(exc)
...
def integration_summary():
    active_source = configured_sales_csv_path()
    active_source_label = sales_source_display_name(active_source)
    ...
    if using_live_sql_server_source() and not live_sql_server_in_cooldown():
        settings = sql_server_settings()
        try:
            min_date, max_date = sql_server_sales_bounds(settings)
            row_count = sql_server_count_sales_rows(settings)
            df = read_csv_sales_data()
            products = df['Product_ID'].nunique() if df is not None and not df.empty else 0
            stores = df['Store_ID'].dropna().nunique() if df is not None and not df.empty and 'Store_ID' in df.columns else 0
            return {
                'active_source': sales_source_display_name(),
                'rows': row_count,
                'products': products,
                'stores': stores,
                'date_range': (
                    f"{min_date.strftime('%Y-%m-%d')} to {max_date.strftime('%Y-%m-%d')}"
                    if min_date is not None and max_date is not None else 'No data loaded'
                ),
                'has_hdpos_fields': True,
                'optional_presence': {column: True for column in OPTIONAL_SALES_COLUMNS},
                'sql_sync_enabled': sql_sync_enabled,
                'sql_last_sync': sql_last_sync,
                'sql_last_status': sql_last_status,
                'sql_interval': sql_interval,
                'live_sql_server_source': True,
            }
        except Exception as exc:
            sql_last_status = f'Live SQL Server summary failed: {exc}'
            remember_live_sql_failure(exc)
            df = read_csv_sales_data()
            if df is not None and not df.empty:
                ...
                return {
                    'active_source': f"{sales_source_display_name()} (using snapshot fallback)",
                    'rows': len(df),
...
def sql_server_settings():
    ...
    return {
        ...
        'port': settings.get('sql_server_port') or '',
        'database': settings.get('sql_server_database') or '',
        'auth_mode': settings.get('sql_server_auth_mode') or 'sql',
...
def save_sql_server_settings_from_form(form):
    fields = {
        ...
        'sql_server_port': form.get('sql_server_port', '').strip(),
        'sql_server_database': form.get('sql_server_database', '').strip(),
        'sql_server_auth_mode': form.get('sql_server_auth_mode', 'sql').strip() or 'sql',
...
def validate_read_only_sql(query):
    compact = ' '.join((query or '').strip().rstrip(';').split())
...
def sql_server_connection_string(settings):
    ...
    if settings.get('port') and '\\' not in server and not server.lower().startswith('np:'):
        server = f"{server},{settings['port']}"
    auth_part = (
        "Trusted_Connection=yes;"
        if settings.get('auth_mode') == 'windows'
        else f"UID={settings['username']};PWD={settings['password']};"
    )
    connect_timeout = int(os.environ.get('SQL_SERVER_CONNECT_TIMEOUT', 15))
    return (
...
def sql_server_base_query(settings):
    query = settings['query'].strip().rstrip(';')
    return re.sub(r'\s+ORDER\s+BY\s+[\s\S]*$', '', query, flags=re.IGNORECASE).strip()

def read_sql_server_sales(settings):
    return read_sql_server_sales_window(settings)

def read_sql_server_dataframe(settings, query, params=None):
    ...
    try:
        ...
        if str(settings.get('encrypt', '')).lower() == 'yes' and 'encryption' in str(exc).lower():
            fallback_settings = dict(settings)
            fallback_settings['encrypt'] = 'no'
            try:
                connection = pyodbc.connect(sql_server_connection_string(fallback_settings))
            except pyodbc.Error:
                fallback_settings['driver'] = 'ODBC Driver 17 for SQL Server'
                connection = pyodbc.connect(sql_server_connection_string(fallback_settings))
        else:
            raise
    query_timeout = int(os.environ.get('SQL_SERVER_QUERY_TIMEOUT', 60))
    try:
...
def read_sql_server_sales_window(settings, start_date=None, end_date=None, limit=None):
    is_valid, validation_message = validate_read_only_sql(settings.get('query', ''))
    if not is_valid:
        raise ValueError(validation_message)

    base_query = sql_server_base_query(settings)
    params = []
    conditions = []
    ...
    if end_date is not None:
        conditions.append("[Date] < DATEADD(day, 1, ?)")
        params.append(pd.to_datetime(end_date).strftime('%Y-%m-%d'))

    top_clause = f"TOP ({max(1, int(limit))}) " if limit else ""
    sql = f"SELECT {top_clause}* FROM ({base_query}) AS sql_source"
    if conditions:
        sql += " WHERE " + " AND ".join(conditions)
    sql += " ORDER BY [Date]"
    return read_sql_server_dataframe(settings, sql, params)

def live_sql_cache_key(settings, start_date=None, end_date=None, limit=None):
    start_key = pd.to_datetime(start_date).strftime('%Y-%m-%d') if start_date is not None else ''
    end_key = pd.to_datetime(end_date).strftime('%Y-%m-%d') if end_date is not None else ''
    settings_key = (
        settings.get('driver', ''),
        settings.get('host', ''),
        settings.get('port', ''),
        settings.get('database', ''),
        settings.get('auth_mode', ''),
        settings.get('username', ''),
        settings.get('encrypt', ''),
        settings.get('trust_certificate', ''),
        hashlib.sha1((settings.get('query') or '').encode('utf-8')).hexdigest(),
    )
    return settings_key, start_key, end_key, limit or ''

def read_normalized_live_sql_sales(settings, start_date=None, end_date=None, limit=None, source_name='SQL Server POS Live'):
    """Read a normalized SQL Server sales window with a short in-process cache.

    SQL Server remains the source of truth; this only prevents every page reload
    from refetching the same selected date window. Stale cache is served on
    transient failures to avoid every request blocking on a slow connection.
    """
    cache_key = live_sql_cache_key(settings, start_date, end_date, limit)
    now = time.time()
    ttl_seconds = max(0, int(app.config.get('LIVE_SQL_CACHE_TTL_SECONDS', 120)))
    stale_ttl = ttl_seconds * 10  # serve stale data for up to 10x TTL on failure
    with LIVE_SQL_DATA_CACHE_LOCK:
        cached = LIVE_SQL_DATA_CACHE.get(cache_key)
        if cached and ttl_seconds and now - cached['created_at'] < ttl_seconds:
            return cached['data'].copy(deep=False)

    try:
        raw_df = read_sql_server_sales_window(settings, start_date, end_date, limit)
        normalized = normalize_sales_dataframe(raw_df, source_name)
    except Exception as exc:
        app.logger.warning("SQL Server read failed, returning stale cache if available: %s", exc)
        with LIVE_SQL_DATA_CACHE_LOCK:
            cached = LIVE_SQL_DATA_CACHE.get(cache_key)
            if cached and stale_ttl and now - cached['created_at'] < stale_ttl:
                return cached['data'].copy(deep=False)
        raise

    if normalized is not None and not normalized.empty and ttl_seconds:
        with LIVE_SQL_DATA_CACHE_LOCK:
            if len(LIVE_SQL_DATA_CACHE) > 24:
                LIVE_SQL_DATA_CACHE.clear()
            LIVE_SQL_DATA_CACHE[cache_key] = {
                'created_at': now,
                'data': normalized,
            }
    return normalized

def cached_live_sql_metadata(cache_name, settings, factory):
    cache_key = (cache_name, live_sql_cache_key(settings)[0])
    now = time.time()
    ttl_seconds = max(0, int(app.config.get('LIVE_SQL_CACHE_TTL_SECONDS', 120)))
    with LIVE_SQL_DATA_CACHE_LOCK:
        cached = LIVE_SQL_METADATA_CACHE.get(cache_key)
        if cached and ttl_seconds and now - cached['created_at'] < ttl_seconds:
            return cached['value']
    ...
    if ttl_seconds:
        with LIVE_SQL_DATA_CACHE_LOCK:
            if len(LIVE_SQL_METADATA_CACHE) > 24:
                LIVE_SQL_METADATA_CACHE.clear()
            LIVE_SQL_METADATA_CACHE[cache_key] = {
                'created_at': now,
                'value': value,
            }
    return value

def sql_server_sales_bounds(settings):
    is_valid, validation_message = validate_read_only_sql(settings.get('query', ''))
    if not is_valid:
        raise ValueError(validation_message)
    ...
    base_query = sql_server_base_query(settings)
    def fetch_bounds():
        bounds = read_sql_server_dataframe(
            settings,
            f"SELECT MIN([Date]) AS MinDate, MAX([Date]) AS MaxDate FROM ({base_query}) AS sql_source"
        )
        if bounds.empty or pd.isna(bounds.iloc[0]['MinDate']) or pd.isna(bounds.iloc[0]['MaxDate']):
            return None, None
        return pd.to_datetime(bounds.iloc[0]['MinDate']), pd.to_datetime(bounds.iloc[0]['MaxDate'])

    return cached_live_sql_metadata('bounds', settings, fetch_bounds)

def sql_server_count_sales_rows(settings, start_date=None, end_date=None):
    base_query = sql_server_base_query(settings)
    params = []
    conditions = []
    if start_date is not None:
    ...
    if end_date is not None:
        conditions.append("[Date] < DATEADD(day, 1, ?)")
        params.append(pd.to_datetime(end_date).strftime('%Y-%m-%d'))
    sql = f"SELECT COUNT_BIG(*) AS TotalRows FROM ({base_query}) AS sql_source"
    if conditions:
        sql += " WHERE " + " AND ".join(conditions)
    def fetch_count():
        rows = read_sql_server_dataframe(settings, sql, params)
        return int(rows.iloc[0]['TotalRows']) if not rows.empty else 0

    return cached_live_sql_metadata(
        f"count:{pd.to_datetime(start_date).strftime('%Y-%m-%d') if start_date is not None else ''}:"
        f"{pd.to_datetime(end_date).strftime('%Y-%m-%d') if end_date is not None else ''}",
        settings,
        fetch_count,
    )
    ...
    with SQL_SERVER_SYNC_LOCK:
        ...
        try:
            ...
            if min_date is None or max_date is None:
                raise ValueError('No rows returned by the SQL Server query.')
            start_date = max_date.normalize() - pd.Timedelta(days=min(SQL_SERVER_DEFAULT_LOOKBACK_DAYS - 1, 13))
            sample_df = read_sql_server_sales_window(settings, start_date, max_date, limit=5000)
            normalized = normalize_sales_dataframe(sample_df, source_name)
            if normalized is None or normalized.empty:
                raise ValueError('No usable sales rows returned. Check query aliases and invoice line fields.')
            total_rows = sql_server_count_sales_rows(settings)
            set_integration_setting('integration_mode', 'sql_server_live')
            set_integration_setting('last_source_name', source_name)
            set_integration_setting('sql_server_last_sync_at', datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            set_integration_setting('sql_server_last_status', f'Live SQL Server active: {total_rows} rows available')
            clear_live_sql_failure()
            ensure_inventory_products_for_sales(normalized)
            log_integration_import(
                source_name,
                ...
                0,
                'success',
                'Live SQL Server source activated. Dashboard reads directly from SQL Server.',
            )
            clear_runtime_caches()
            return True, f'Live SQL Server source activated. {total_rows} rows available; dashboard now reads SQL Server directly.'
        except Exception as exc:
            message = f'SQL Server activation failed: {exc}'
...
def snapshot_sql_server_sales(source_name='SQL Server POS Snapshot'):
    with SQL_SERVER_SYNC_LOCK:
        settings = sql_server_settings()
        try:
            raw_df = read_sql_server_sales_window(settings)
            normalized = normalize_sales_dataframe(raw_df, source_name)
            if normalized is None or normalized.empty:
                raise ValueError('No usable sales rows returned. Check query aliases and invoice line fields.')
...
def apply_date_filter(df):
    ...
    min_date = date_values.min().normalize()
    max_date = date_values.max().normalize()
    start_date = min_date
    end_date = max_date

    if start_date_str and end_date_str:
        try:
            start_date = max(pd.to_datetime(start_date_str).normalize(), min_date)
            end_date = min(pd.to_datetime(end_date_str).normalize(), max_date)
    ...
    normalized_dates = date_values.dt.normalize()
    filtered_df = df[(normalized_dates >= start_date) & (normalized_dates <= end_date)]
...
def sql_server_request_range(settings):
    min_date, max_date = sql_server_sales_bounds(settings)
    if min_date is None or max_date is None:
        return None
    ...
    if start_date_str and end_date_str:
        try:
            ...
            start_date = max(max_date - pd.Timedelta(days=SQL_SERVER_DEFAULT_LOOKBACK_DAYS - 1), min_date)
            end_date = max_date
    else:
        start_date = max(max_date - pd.Timedelta(days=SQL_SERVER_DEFAULT_LOOKBACK_DAYS - 1), min_date)
        end_date = max_date
...
def build_invoice_analytics(df):
    ...
    if invoice_df.empty:
        return {
            ...
            'invoice_bundle_lookback_days': BUNDLE_SIGNAL_LOOKBACK_DAYS,
        }
    ...
    recent_cutoff = invoice_df['Date'].max() - pd.Timedelta(days=max(1, BUNDLE_SIGNAL_LOOKBACK_DAYS) - 1)
    recent_df = invoice_df[invoice_df['Date'] >= recent_cutoff]
    bundle_candidates = calculate_invoice_bundle_candidates(recent_df)
    basket_invoice_count = int(
        build_invoice_line_frame(recent_df)
        .groupby('Invoice_No')['Product_Name']
        .nunique()
        .ge(2)
        .sum()
    ) if not recent_df.empty else 0
    bundle_threshold = scaled_bundle_pair_threshold(basket_invoice_count)
    ...
    return {
        ...
        'invoice_bundle_threshold': bundle_threshold,
        'invoice_bundle_lookback_days': BUNDLE_SIGNAL_LOOKBACK_DAYS,
...
def scaled_bundle_pair_threshold(total_basket_invoices):
    if total_basket_invoices <= 0:
        return 0
    return max(2, int(np.ceil(total_basket_invoices * 0.005)))

def calculate_invoice_bundle_candidates(df, min_pair_count=None, limit=12):
    invoice_df = build_invoice_line_frame(df)
    ...
    if total_invoices == 0:
        return []
    if min_pair_count is None:
        min_pair_count = scaled_bundle_pair_threshold(total_invoices)
    min_confidence = 0.05

    product_invoice_counts = {}
    pair_counts = {}
    pair_sales = {}
    invoice_sales = invoice_df.groupby('Invoice_No')['Total_Sale'].sum().astype(float).to_dict()
    for _, row in invoice_products.iterrows():
        products = row['products']
        invoice_sale = invoice_sales.get(row['Invoice_No'], 0.0)
    ...
    for (product_a, product_b), pair_count in pair_counts.items():
        ...
        best_confidence = max(confidence_ab, confidence_ba)
        if best_confidence < min_confidence:
            continue
        support = pair_count / total_invoices
        lift_ab = confidence_ab / (b_count / total_invoices) if b_count else 0
        lift_ba = confidence_ba / (a_count / total_invoices) if a_count else 0
        if confidence_ab >= confidence_ba:
            anchor_product = product_a
            add_on_product = product_b
            lift = lift_ab
            anchor_invoice_count = a_count
            add_on_invoice_count = b_count
            anchor_confidence = confidence_ab
        else:
            anchor_product = product_b
            add_on_product = product_a
            lift = lift_ba
            anchor_invoice_count = b_count
            add_on_invoice_count = a_count
            anchor_confidence = confidence_ba
        recommendation = (
            f"Bundle {anchor_product} with {add_on_product}; they appeared together in "
            f"{pair_count} invoices during the last {BUNDLE_SIGNAL_LOOKBACK_DAYS} days."
        )
        candidates.append({
            'product_a': product_a,
            'product_b': product_b,
            ...
            'basket_invoice_count': int(total_invoices),
            'support': support,
            'confidence_ab': anchor_confidence,
            'confidence_pct': anchor_confidence * 100,
            'anchor_invoice_count': int(anchor_invoice_count),
            'add_on_invoice_count': int(add_on_invoice_count),
            'lift_ab': lift,
...
def build_forecast_owner_plan(context, ai_decision):
    ...
    first_margin_action = next(
        (item for item in product_actions if item.get('action_type') in {'Bundle', 'Optimize margin', 'Review margin', 'Protect price'}),
        None,
    )
    return {
...
def daily_totals(df, start_date=None, end_date=None):
    invoice_df = build_invoice_line_frame(df)
    if invoice_df.empty:
        return pd.DataFrame(columns=['Date', 'Total_Sale', 'Total_Profit', 'Units_Sold', 'Invoice_Count'])
    invoice_df['Date'] = pd.to_datetime(invoice_df['Date'], errors='coerce').dt.normalize()
    invoice_df = invoice_df.dropna(subset=['Date'])
    if invoice_df.empty:
        return pd.DataFrame(columns=['Date', 'Total_Sale', 'Total_Profit', 'Units_Sold', 'Invoice_Count'])
...
def build_charts(df):
    ...
    product_performance = cached_product_performance(df)
...
def build_period_movement(source_df, start_date, end_date, selected_category=''):
    ...
    comparison_dates = pd.to_datetime(comparison_df['Date'], errors='coerce').dt.normalize()
    current_df = comparison_df[(comparison_dates >= start_date) & (comparison_dates <= end_date)]
    previous_df = comparison_df[(comparison_dates >= previous_start) & (comparison_dates <= previous_end)]
...
def data_context():
    if using_live_sql_server_source() and not live_sql_server_in_cooldown():
        try:
            settings = sql_server_settings()
            range_context = sql_server_request_range(settings)
            if range_context is None:
                return None, None
            df = read_normalized_live_sql_sales(
                settings,
            ...
            return df, {
                **range_context,
                **cached_metrics(df, range_context['start_date'], range_context['end_date']),
                **cached_charts(df),
                **cached_product_performance(df),
                **inventory_metrics(inventory_items),
                'live_sql_server_source': True,
            }
        except Exception as exc:
            app.logger.warning("Live SQL Server dashboard read failed; using snapshot: %s", exc)
            remember_live_sql_failure(exc)
    ...
    return filtered_df, {
        **range_context,
        **cached_metrics(filtered_df, range_context['start_date'], range_context['end_date']),
        **cached_charts(filtered_df),
        **cached_product_performance(filtered_df),
        **inventory_metrics(inventory_items),
    }
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not _check_auth_rate_limit(client_ip):
            flash('Too many login attempts. Please wait a few minutes before trying again.', 'danger')
...
def signup():
    ...
    if form.validate_on_submit():
        client_ip = request.headers.get('X-Forwarded-For', request.remote_addr or '').split(',')[0].strip()
        if not _check_auth_rate_limit(client_ip):
            flash('Too many registration attempts. Please wait a few minutes before trying again.', 'danger')
...
def plotly_js():
    ...
    resp = make_response(send_file(plotly_path, mimetype='application/javascript'))
...
def calculate_inventory_optimization(df):
    ...
    # Derive a deterministic stock estimate from real inventory when available,
    # falling back to a fixed 10-day cover so results are reproducible.
    inventory = pd.DataFrame(load_inventory_items())
    if not inventory.empty:
        inventory_stock = inventory.rename(columns={
            'product_name': 'Product_Name', 'category': 'Category', 'current_stock': 'current_stock'
        })[['Product_Name', 'Category', 'current_stock']]
        daily_demand = daily_demand.merge(inventory_stock, on=['Product_Name', 'Category'], how='left')
        daily_demand['current_stock'] = daily_demand['current_stock'].fillna(daily_demand['avg_daily_demand'] * 10)
    else:
        daily_demand['current_stock'] = daily_demand['avg_daily_demand'] * 10
...
def calculate_waste_reduction(df):
    ...
    def get_recommended_action(row):
        ...
        if row['waste_risk'] == 'Expired':
            ...
            if int(row['is_perishable']) == 1:
                return f'Feature this item for the next {int(round(days)) if days else 3} days and reduce the next reorder if cover remains high'
...
def calculate_dynamic_pricing_opportunities(df):
    ...
    if df is None or len(df) == 0:
        return []
    has_bundle_signals = bool(calculate_cross_selling_opportunities(df))
    ...
    def get_pricing_opportunity(row):
        ...
        # Low margin and strong volume: only recommend bundles when invoice baskets prove pairings.
        elif row['profit_margin_pct'] < 25 and row['Units_Sold'] >= high_volume_cutoff:
            return 'bundle_margin' if has_bundle_signals else 'margin_review'
    ...
    def get_suggested_action(row):
        ...
        elif row['pricing_opportunity'] == 'margin_review':
            return f'Review supplier cost, pack size, or a small price test for this high-volume low-margin item; no proven bundle pair exists yet'
...
def forecast_product_units(df, product_name):
    ...
    product_df['Date'] = pd.to_datetime(product_df['Date'], errors='coerce').dt.normalize()
    product_daily = product_df.groupby('Date', as_index=False).agg({'Units_Sold': 'sum'})
...
def build_seller_decision_intelligence(df, context):
    ...
    date_span = max((df['Date'].max() - df['Date'].min()).days + 1, 1)
    trusted_stock_columns = {'Current_Stock', 'Reorder_Level'}.issubset(df.columns)
    trusted_stock_available = (
        trusted_stock_columns and
        (pd.to_numeric(df['Current_Stock'], errors='coerce').notna().any() or
         pd.to_numeric(df['Reorder_Level'], errors='coerce').notna().any())
    )
    inventory = pd.DataFrame(load_inventory_items())
    if not inventory.empty:
        inventory = inventory.rename(columns={
    ...
    if trusted_stock_available:
        sales_agg['Current_Stock'] = 'max'
        sales_agg['Reorder_Level'] = 'max'

    product_stats = df.groupby(['Product_ID', 'Product_Name', 'Category'], as_index=False).agg(sales_agg)
    product_stats['Has_Trusted_Stock'] = trusted_stock_available
    if not inventory.empty:
        ...
        for column in ['Current_Stock', 'Reorder_Level']:
            inventory_column = f'{column}_Inventory'
            if column not in product_stats.columns:
                product_stats[column] = product_stats[inventory_column] if inventory_column in product_stats.columns else np.nan
            elif inventory_column in product_stats.columns:
                product_stats[column] = product_stats[column].fillna(product_stats[inventory_column])

        missing_stock = product_stats['Current_Stock'].isna()
        if missing_stock.any() and trusted_stock_available:
            fallback_inventory = (
                inventory.groupby(['Product_Name', 'Category'], as_index=False)
                .agg({'Current_Stock': 'sum', 'Reorder_Level': 'max'})
                .rename(columns={
    ...
    pricing_lookup = {
        ...
        for item in cached_pricing_opportunities(df)
    }
    has_bundle_signals = bool(cached_cross_selling(df))
    ...
    for _, row in product_stats.iterrows():
        ...
        has_trusted_stock = bool(row.get('Has_Trusted_Stock'))
        ...
        if has_trusted_stock and stock <= 0 and forecast_units > 0:
        ...
        elif has_trusted_stock and forecast_units > stock and forecast_units > 0:
        ...
        elif has_trusted_stock and stock <= reorder_level and daily_velocity > 0:
            ...
            if has_bundle_signals:
                action_type = 'Bundle'
                suggested_action = f"Keep the ${unit_price:.2f} shelf price and bundle with a proven high-margin add-on to lift basket profit."
                reason = 'high demand, low margin, and invoice bundle signal'
            else:
                action_type = 'Review margin'
                suggested_action = f"Review cost, pack size, or a small price test for {product_name}; no proven bundle pair exists in invoices."
                reason = 'high demand but low margin; no proven bundle pair'
            priority = 78
            confidence = 0.79
            impact = revenue * 0.10
        elif has_trusted_stock and units <= unit_low and stock > max(forecast_units * 2, reorder_level * 2) and daily_velocity > 0:
        ...
        product_actions.append({
            ...
            'current_stock': stock,
            'has_trusted_stock': has_trusted_stock,
    ...
    if invoice_bundles:
        ...
        summary.append(
            ...
            f"{top_bundle['pair_count']} times in the last {BUNDLE_SIGNAL_LOOKBACK_DAYS} days; make it a promoted bundle."
        )
    if category_actions:
...
def calculate_cross_selling_opportunities(df):
    ...
    if 'Invoice_No' in invoice_df.columns and invoice_df.groupby('Invoice_No')['Product_Name'].nunique().max() >= 2:
        invoice_candidates = calculate_invoice_bundle_candidates(invoice_df, limit=20)
        if invoice_candidates:
            return invoice_candidates
        return []

    # Fall back only for older demo data that lacks true basket IDs entirely.
    if 'Invoice_No' in invoice_df.columns and invoice_df['Invoice_No'].fillna('').astype(str).str.strip().ne('').any():
        return []
...
def build_dynamic_insights(df, context):
    ...
    if invoice_bundles:
        ...
        add(
            ...
            f"{top_bundle['anchor_product']} and {top_bundle['add_on_product']} were purchased together in {top_bundle['pair_count']} invoices over the last {BUNDLE_SIGNAL_LOOKBACK_DAYS} days. Create a bundle, shelf pairing, or checkout prompt.",
            'View Cross-Sell',
    ...
    waste_data = cached_waste_reduction(df)
    ...
    pricing_data = cached_pricing_opportunities(df)
    action_pricing = [item for item in pricing_data if item.get('pricing_opportunity') != 'hold_price']
    if action_pricing:
        priority_order = {'price_increase': 5, 'bundle_margin': 4, 'margin_review': 4, 'reduce_or_replace': 3, 'visibility_push': 2, 'protect_price': 1}
        best_pricing = sorted(action_pricing, key=lambda item: priority_order.get(item.get('pricing_opportunity'), 0), reverse=True)[0]
        add(
            74,
            'warning' if best_pricing['pricing_opportunity'] in {'bundle_margin', 'margin_review', 'reduce_or_replace'} else 'success',
            'tags',
    ...
    cross_sell_data = cached_cross_selling(df)
    if cross_sell_data:
...
def analytics():
    ...
    context = {
        **range_context,
        **cached_metrics(analytics_df, range_context['start_date'], range_context['end_date']),
        **movement_context,
        **cached_charts(analytics_df),
        **cached_product_performance(analytics_df),
...
def inventory():
    ...
    inventory_items = load_inventory_items()
    performance = cached_product_performance(filtered_df)
    if filtered_df is not None and not filtered_df.empty:
        sales_by_product = (
            filtered_df.groupby('Product_ID', as_index=False)['Units_Sold']
            .sum()
            .set_index('Product_ID')['Units_Sold']
            .to_dict()
        )
    else:
        sales_by_product = {}
    ...
    for item in inventory_items:
        units_sold = int(_safe_int(sales_by_product.get(item['product_id'], 0)))
        current_stock = int(item['current_stock'])
...
def waste():
    ...
    filtered_df, range_context = apply_date_filter(df)
    waste_data = cached_waste_reduction(filtered_df)
    
    return render_template('waste.html',
        waste_data=waste_data,
        period_label=range_context['period_label'],
        start_date=range_context['start_date'],
        end_date=range_context['end_date']
    )
...
def pricing():
    ...
    filtered_df, range_context = apply_date_filter(df)
    pricing_data = [dict(item) for item in cached_pricing_opportunities(filtered_df)]
    ai_context = {
        **range_context,
        **cached_metrics(
            filtered_df,
    ...
    for item in pricing_data:
        ...
        if seller_action:
            action_type = seller_action.get('action_type')
            action_to_opportunity = {
                'Increase': 'price_increase',
                'Bundle': 'bundle_margin',
                'Review margin': 'margin_review',
                'Review': 'reduce_or_replace',
                'Promote': 'visibility_push',
                'Protect price': 'protect_price',
                'Hold': 'hold_price',
            }
            opportunity_labels = {
                'price_increase': 'Increase',
            ...
            if action_type in action_to_opportunity:
                item['pricing_opportunity'] = action_to_opportunity[action_type]
                item['opportunity_label'] = opportunity_labels[item['pricing_opportunity']]
                item['suggested_action'] = seller_action['suggested_action']
...
def cross_sell():
    ...
    filtered_df, range_context = apply_date_filter(df)
    cross_sell_data = cached_cross_selling(filtered_df)
...
def api_data_status():
    mode = get_integration_setting('integration_mode', 'sample_csv')
    in_cooldown = live_sql_server_in_cooldown()
    last_status = get_integration_setting('sql_server_last_status', '')
    last_sync = get_integration_setting('sql_server_last_sync_at', '')
    cache_count = 0
    with LIVE_SQL_DATA_CACHE_LOCK:
        cache_count = len(LIVE_SQL_DATA_CACHE)
    return jsonify({
        'mode': mode,
        'live_sql': mode == 'sql_server_live',
...
def page_not_found(exc):
    ...
    return render_template('error.html', message='Page not found. Please use the navigation menu.'), 404

@app.errorhandler(413)
def request_too_large(exc):
    flash('The uploaded file is too large. Maximum size is 64 MB.', 'danger')
    return redirect(request.referrer or url_for('integration'))
